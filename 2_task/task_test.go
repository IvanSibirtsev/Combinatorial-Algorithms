...
func TestPositions(t *testing.T) {
	...
	dfs, testChessBoard := getNewTestObjects()
...
func TestDeltas(t *testing.T) {
	pawn := ChessFigure{7, 'a'}
	knight := ChessFigure{3, 'c'}
	dfs, testChessBoard := getNewTestObjects()
	dfs.run(testChessBoard, knight, pawn)
	expected := "c3 b5 a7"
	assertEquals(dfs.result, expected, t)

	pawn = ChessFigure{7, 'e'}
	dfs, testChessBoard = getNewTestObjects()
	dfs.run(testChessBoard, knight, pawn)
	expected = "c3 b5 a7 c8 e7"
	assertEquals(dfs.result, expected, t)

	knight = ChessFigure{1, 'h'}
	dfs, testChessBoard = getNewTestObjects()
	dfs.run(testChessBoard, knight, pawn)
	expected = "h1 g3 f5 e7"
	assertEquals(dfs.result, expected, t)
}
...
func getNewTestObjects() (DFS, []map[int32]bool) {
	testChessBoard := makeChessBoard()
	dfs := DFS{}
	return dfs, testChessBoard
}
